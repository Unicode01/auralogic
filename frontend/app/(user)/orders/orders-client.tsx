...
import { buildUpdatedQueryString, normalizeQueryString } from '@/lib/query-state'
import {
...
function OrdersPageContent() {
  ...
  const initialQuery = parseOrderListSearchParams(searchParams)
  const initialSearch = initialQuery.search
  const initialStatus = initialQuery.status
  ...
  useEffect(() => {
    const nextQuery = parseOrderListSearchParams(searchParams)
    const nextSearch = nextQuery.search
    const nextStatus = nextQuery.status
    ...
    if (isMobile && !isRestoringPages && orders.length > 0) {
      if (page === 1) {
        ...
        setAllOrders((prev) => {
          ...
          const newOrders = orders.filter((order: Order) => !existingIds.has(order.id))
  ...
  const userOrdersPluginContext = {
    ...
    pagination: {
      page,
      limit: pagination?.limit || ORDER_LIST_LIMIT,
