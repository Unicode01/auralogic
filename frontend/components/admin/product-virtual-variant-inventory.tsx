...
import { FileText, Sparkles, AlertCircle, Database } from 'lucide-react'
import { useLocale } from '@/hooks/use-locale'
import { getTranslations } from '@/lib/i18n'

export interface VirtualVariantInventoryBinding {
...
interface ProductVirtualVariantInventoryProps {
    attributes: Array<{ name: string; values: string[]; mode: 'user_select' | 'blind_box' }>
    variantMode: 'user_select' | 'blind_box'
    bindings: VirtualVariantInventoryBinding[]
    virtualInventories: any[]
    onBindingsChange: (bindings: VirtualVariantInventoryBinding[]) => void
}

export function ProductVirtualVariantInventory({
    attributes,
    variantMode,
    bindings,
    virtualInventories,
    onBindingsChange,
}: ProductVirtualVariantInventoryProps) {
    ...
    useEffect(() => {
        ...
        const newBindings: VirtualVariantInventoryBinding[] = uniqueVariants.map(attrs => {
    ...
    const updateBinding = useCallback((
        index: number,
        field: keyof VirtualVariantInventoryBinding,
        value: any
    ) => {
        const updated = [...bindings]
        if (field === 'virtual_inventory_id') {
            updated[index].virtual_inventory_id = value ? parseInt(value) : null
    ...
    const allConfigured = useMemo(() => {
        return bindings.every(b => b.virtual_inventory_id !== null)
    }, [bindings])
            ...
            <CardHeader>
                <CardTitle className="flex items-center justify-between">
                    <span className="flex items-center gap-2">
                        ...
                        {t.admin.virtualVariantInventoryConfig}
                    </span>
                    ...
                    <>
                        {!allConfigured && (
                            <div className="flex items-center gap-2 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg text-yellow-800 dark:text-yellow-300">
                                ...
                                <p className="text-sm">
                                    {isDefaultProduct ? t.admin.configDefaultVirtualProduct : t.admin.configAllVirtualVariants}
                                </p>
                            </div>
                        )}
                        ...
                        <div className="space-y-3">
                            <Label>
                                {isDefaultProduct
                                    ? t.admin.noSpecVirtualInventory
                                    : t.admin.virtualVariantInventoryCount.replace('{count}', String(bindings.length))}
                            </Label>
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead>{t.admin.virtualInventoryConfig}</TableHead>
                                ...
                                <TableBody>
                                    {bindings.map((binding, index) => {
                                        ...
                                        const inventory = getInventoryInfo(binding.virtual_inventory_id)
                                        const probability =
                                        ...
                                        return (
                                            <TableRow key={index}>
                                                ...
                                                <TableCell>
                                                    <Select
                                                        value={binding.virtual_inventory_id?.toString() || ''}
                                                        onValueChange={(value) =>
                                                            updateBinding(index, 'virtual_inventory_id', value)
                                                        }
                                                    >
                                                        <SelectTrigger className="w-[300px]">
                                                            <SelectValue placeholder={t.admin.selectVirtualInventoryOption} />
                                                        </SelectTrigger>
                                                        <SelectContent>
                                                ...
                                                <TableCell>
                                                    {isBlindBox ? (
                                                        <Input
                                                            ...
                                                            className="w-20"
                                                            disabled={!binding.virtual_inventory_id}
                                                        />
                                                    ) : (
                                                ...
                                                <TableCell>
                                                    {isBlindBox && binding.virtual_inventory_id ? (
